...
Коли ми передаємо дані через інтернет, ми хочемо їх зашифрувати, щоб ніхто посторонній не зміг прочитати вміст наших даних. Уявімо собі що ви хочете відправити зашифровані дані своєму другу, відповідно він має їх розшифровувати. Для цього нам потрібен ключ який знатимете і ви, і ваш друг. Ви хочете передати ключ другу, але якщо зловмисник побачить цей ключ в процесі передачі то зможе читати подальші повідомлення. Ви не можете зашифрувати свій ключ бо саме цей ключ і потрібен вашему другу для розшифрування даних. Невже нам потрібно фізично зустрітись і поділитись ключем з другом щоб передавати зашифровані дані через інтернет?  
...
`g` - число "генератор". Це число має бути відомим обом сторонам і може передаватись через інтернет. Зловмисник може його бачити, в цьому немає нічого страшного.  
...
2. Аліса розраховує "публічне" число `ag (помаранчеве)` шляхом змішування публічного генератора `g (жовте)` та особистого секретного `a`. Боб робить те саме, тільки зі своїм особистим секретним `b` та публічним `g` та розраховує `bg (зелене)`.
...
`abg` - це фінальний спільний секрет, який Боб та Аліса самостійно розраховують, використовуючи математичну властивість $(g^a)^b = (g^b)^a$. Боб бере `ag` Аліси, і зводить його у степінь свого секретного `b` ось так: `abg = ag^b`. В реальності це виглядає як $abg = (g^a)^b$. В цей час Аліса бере `bg` Боба, взводить його у свою секретну степінь `a` та розраховує його як $abg = (g^b)^a$.  

Математичне обчислення:  
Для зручності можна використати [онлайн калькулятор зведення у степінь по модулю](https://www.omnicalculator.com/math/power-modulo)  
...
8. Боб отримав `ag = 390` від Аліси і розраховує спільний секрет `abg`. $abg = ag^b mod (n)$;  $abg = 390^b mod (997) = 247$
9. Причина чому це спрацювало в тому, що $(53^a)^b mod (n) = (53^b)^a mod (n)$.    

### 1.3 Еліптичні криві та DH
Як ми вивчали на минулому занятті, асиметрична криптографія має новий дуже потужний математичний фундамент - еліптичні криві. Вони швидші та мають більшу криптостійкість в порівнянні з іншими асиметричними криптографічними системами. Алгоритм DH - не виключення. Існує ECDH, тобто Elliptic Curves Diffie-Hellman, як алгоритм узгодження спільного секрету через незахищений канал. Звичайно, при використанні алгоритму DH треба обирати його версію на еліптичних кривих. Такий алгоритм може позначатись як ECDH, X25519 (від назви еліптичної кривої), ECDHE (коли автентифікація робиться іншими інструментами).

## 2. Додаткові матеріали
1. [Відео зі спрощеним поясненням Diffie-Hellman](https://www.youtube.com/watch?v=NmM9HA2MQGI)
2. [Відео з математичним поясненням Diffie-Hellman](https://www.youtube.com/watch?v=Yjrfm_oRO0w)
3. [Крута стаття з поясненням RSA, Diffie-Hellman, та DSA](https://www.practicalnetworking.net/practical-tls/rsa-diffie-hellman-dsa-asymmetric-cryptography-explained/#:~:text=The%20RSA%20Algorithm%20can%20do,only%20be%20used%20for%20Signatures.)  
