...
`ag` - це число яке Аліса передає Бобу. Воно розраховується як $g^a mod n$. Боб знає `g` та `n`, він знає `ag`, він не може порахувати секретне `a` Аліси, але може робити математичні операції з `ag`. Це число відомо зловмиснику.   
`bg` - це число яке Боб передає Алісі. Воно розраховується як $g^b mod n$. Аліса знає `g` та `n`, вона знає `bg`, вона не може порахувати секретне `b` Боба, але може робити математичні операції з `bg`. Це число відомо зловмиснику.  
