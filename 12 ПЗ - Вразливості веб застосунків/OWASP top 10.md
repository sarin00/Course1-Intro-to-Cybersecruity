...
Веб застосунки - це програми, які зазвичай доступні всьому інтернету і які працюють з HTTP. Як і будь-яка програма, веб застосунок може містити у собі вразливості. 
...
`1. Broken Access Control`  
Виникає коли системі не вдається належним чином обмежити доступ користувачів до певних функцій або даних.  
...
Приклади:  
SQL Injection: ін'єкція SQL-стейтменту через user input, що дозволяє зловмиснику виконувати свої команди для бази даних.  
...
2. В налаштуваннях Burp Suite зайти в Intercept, впевнитись що перехват запитів включено (Intercept is on)  
3. Ввести будь-який логін та будь-який пароль, та натиснути кнопку "Log in"
...
8. Через змінений запит до бази даних, система залогінює нас як першого користувача у списку (першого створеного користувача у системі), в якого за щасливим збігом обставин є адміністративні привілеї.

### 4. Bad access control
1. Переййти на сторінку Check out our boring terms of use if you are interested in such lame stuff зі сторінки About Us
2. З директорії /ftp/legal.md перейти в /ftp
3. Подивитись "конфіденційні" документи

### 5. Registering a user with admin privileges
1. Перейти на сторінку реєстрації нового користувача  
2. Ввести всі дані, включити Intercept в Burp Suite, та відправити запит  
3. Проаналізувати JSON тіло запиту  
4. В тілі JSON додати role:admin ` {"email":"admin","password":"admin","role":"admin"}`  
5. Відправити  

### 6. Brute Force account credentials  
1. Піти на сторінку логіну  
2. Використати пошту `admin@juice-sh.op` та ввести будь-що як пароль  
3. Включити Intercept в Burp Suite  
4. Натиснути кнопку Log in  
5. Перенаправити запит в Intruder  
6. Поставити змінну для значення паролю  
7. Перейти у вкладку Payloads, додати наступний список паролів:  
```
qwerty123
p@$$word
admin123
...
```
9. Натиснути "Attack"  
10. Проаналізувати response codes  
...
2. Пройти всі завдання в Juice Shop. Якщо це ваші перші кроки у пентесті веб застосунків, не цурайтесь читати та дивитись відео з детальними проходженнями та поясненнями цих завдань. До того як ви зможете самостійно знаходити та експлуатувати вразливості, вам потрібно спочатку побачити і зрозуміти як це має працювати  
...
2. [OWASP cheatsheets top 10](https://cheatsheetseries.owasp.org/IndexTopTen.html)  
3. [Сайт з інтерактивними прикладами веб вразливостей hacksplaining.com](https://hacksplaining.com/owasp)  
4. [Сайт MITRE зі списком CVE](https://cve.mitre.org/cve/)  
5. [Інструкції для самостійного встановлення juiceshop](https://pwning.owasp-juice.shop/companion-guide/latest/part1/running.html)  
6. [Документація Burp Suite](https://portswigger.net/burp/documentation/desktop/getting-started)  
