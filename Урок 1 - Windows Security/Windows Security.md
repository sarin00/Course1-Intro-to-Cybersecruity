...
Ви можете натиснути правою кнопкою миші на службу, щоб отримати більше інформації про неї, запустити / зупинити службу або змінити її налаштування.
...
PowerShell - це інструмент для автоматизації адміністративних завдань, якеий складається з командного рядка, синтаксису PowerShell та його команд (командлетів). PowerShell працює на Windows, Linux та macOS.
...
Групова політика (Group Policy) - це набір конфігурацій які змінюють налаштування операційної системи. Такі конфігурації пожна робити як для одного компʼютера (local group policy), так і для цілого домену через Active Directory Domain Services (AD DS) за допомогою Group Policy Management Console (GPMC).
...
3. Правою кнопкою миші по "Turn on Module Logging", обрати Edit, поставити галку біля "Enabled"
...
8. Натисніть правою кнопкою миші на "Turn on PowerShell Script Block Logging" та оберіть "Edit"
...
3. Дозвольте входящий ICMP і перевірте мережеву доступність до цієї OS Windows командою ping з іншого компʼютера в одній мережі з Windows
